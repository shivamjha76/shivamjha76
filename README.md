<div align="center">

<img src="./assets/banner.png" alt="Code Learn Build Banner" width="100%">

</div>

👋 Hey, I'm Shivam jha

💻 Computer Science Engineering Student | Backend & Web Development | Core CS

I'm a B.Tech Computer Science Engineering student at Jagan Nath University, Jaipur, currently building strong foundations in Core Computer Science and practical software development.

I enjoy turning ideas into working projects, learning how systems work under the hood, and continuously improving my problem-solving skills.

---

🚀 About Me

- 🎓 B.Tech Computer Science Engineering student
- 💻 Currently strengthening Core CS & DSA
- 🧩 Practicing C++ & problem solving
- 🌐 Exploring Web & Backend Development
- 🗄️ Learning DBMS, SQL & PostgreSQL
- ⚡ Building projects with FastAPI & modern web technologies
- 🛠️ Interested in building practical, real-world applications
- 📚 Currently focused on becoming a stronger software engineer

---

🧠 Current Focus

Core CS
├── Data Structures & Algorithms
├── DBMS
├── Operating Systems
├── Computer Networks
└── Computer Architecture

Development
├── C++
├── Python
├── FastAPI
├── SQL / PostgreSQL
├── Git & GitHub
└── Web Development

---

🛠️ Tech Stack

Languages

<p>
  <img src="https://skillicons.dev/icons?i=cpp,c,python,js" />
</p>Web & Backend

<p>
  <img src="https://skillicons.dev/icons?i=html,css,js,react,nextjs,fastapi" />
</p>Database & Tools

<p>
  <img src="https://skillicons.dev/icons?i=postgres,mysql,git,github,vscode,postman" />
</p>---

📌 Featured Projects

🏆 HackVerse — Online Hackathon Platform

A platform designed to manage hackathons digitally with multiple user roles and structured workflows.

Tech: FastAPI • PostgreSQL • SQLAlchemy • Alembic • Next.js

Highlights

- 👤 Participant, Organizer, Judge & Admin roles
- 🔐 Authentication & authorization
- 🗄️ PostgreSQL database
- ⚙️ REST APIs with FastAPI
- 📊 Dashboard-oriented architecture
- 🚀 Built with scalability and maintainability in mind

---

🤖 College Knowledge-Based Chatbot

A college information chatbot built around structured university data.

Focus: Knowledge Base • JSON • Web Data • English & Hinglish

Highlights

- 📚 Structured college information
- 🔎 Keyword-based question matching
- 💬 English + Hinglish support
- 💡 Suggested follow-up questions
- 🧩 Designed for future AI integration

---

💻 DSA & Problem Solving

A growing collection of C++ solutions and DSA practice focused on developing strong problem-solving fundamentals.

Topics include:

"Arrays" • "Strings" • "Functions" • "Stack" • "Searching" • "Sorting" • "Recursion" • "STL"

---

📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=transparent&hide_border=true" height="170"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_USERNAME&layout=compact&theme=transparent&hide_border=true" height="170"/>
</p><p align="center">
  <img src="https://streak-stats.demolab.com?user=YOUR_USERNAME&theme=transparent&hide_border=true" />
</p>---

🏆 GitHub Achievements

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=YOUR_USERNAME&theme=flat&no-frame=true&no-bg=true&margin-w=8" />
</p>---

📈 Contribution Graph

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=YOUR_USERNAME&theme=github-compact&hide_border=true" />
</p>---

🌱 Currently Learning

C++ & DSA                 █████████░  Building Strong Foundations
Core CS                   ████████░░  In Progress
DBMS & SQL                ████████░░  In Progress
Backend Development       ███████░░░  Growing
Web Development           ██████░░░░  Exploring
System Design              ████░░░░░░  Exploring

---

🎯 2026 Goals

- [ ] Build strong DSA fundamentals
- [ ] Solve more problems consistently
- [ ] Strengthen Core CS concepts
- [ ] Build production-quality projects
- [ ] Improve backend development skills
- [ ] Contribute to open source
- [ ] Build a strong GitHub portfolio

---

🤝 Let's Connect

<p align="left">
  <a href="www.linkedin.com/in/shivam-jha-785b92387">
    <img src="https://skillicons.dev/icons?i=linkedin" width="45"/>
  </a>
  &nbsp;
  <a href="[https://github.com/YOUR_USERNAME](https://github.com/shivamjha76)">
    <img src="https://skillicons.dev/icons?i=github" width="45"/>
  </a>
</p>---

<p align="center">
  <i>“Learning consistently. Building continuously. Improving every day.”</i>
</p><p align="center">
  ⭐ If you find my projects useful, consider giving them a star!
</p>
